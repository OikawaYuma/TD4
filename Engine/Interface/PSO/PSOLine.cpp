...
void PSOLine::CreatePipelineStateObject() {
	// DirectXCommonのインスタンスを取得
	DirectXCommon* sDirectXCommon = DirectXCommon::GetInstance();

	PSOLine::CreateRootSignature();
	PSOLine::SetInputLayout();
	PSOLine::SetBlendState();
	PSOLine::SetRasterrizerState();
	PSOLine::CreateDepth();
	// Shaderをコンパイルする
	property.vertexShaderBlob = CompileShader(L"Resources/shader/Line.VS.hlsl",
	...
	property.pixelShaderBlob = CompileShader(L"Resources/shader/Line.PS.hlsl",
...
void PSOLine::CreateRootSignature() {
	// DirectXCommonのインスタンスを取得
	DirectXCommon* sDirectXCommon = DirectXCommon::GetInstance();
	...
	descriptorRange_[0].BaseShaderRegister = 0; // 1から始まる
	...
	rootParamerters[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX; // PixelShaderで使う
...
void PSOLine::SetRasterrizerState() {
	//裏面（時計回り）を表示しない
...
PSOLine* PSOLine::GatInstance() {
	static PSOLine instance;
	return &instance;
}
